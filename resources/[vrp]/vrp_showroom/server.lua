                        ...
                        if vehgarage.vehicles[i].model == vehicle and vehgarage.vehicles[i].costs == price then
                            if price > 5000 then
                                if vRP.tryFullPayment({user_id,price}) then
                                    ...
                                    PerformHttpRequest(webhook.BuyCar, function(err, text, headers) end, 'POST', json.encode({username = "SenturaCloud - Logs", content = "**"..user_id.."** har lige købt en **"..vehicle.."** for: **"..format_thousand(price).."**"}), { ['Content-Type'] = 'application/json' })
                                else
