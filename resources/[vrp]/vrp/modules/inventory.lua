          ...
          vRP.prompt(player,lang.inventory.give.prompt({vRP.getInventoryItemAmount(user_id,idname)}),"",function(player,amount)
            ...
            if new_weight <= vRP.getInventoryMaxWeight(nuser_id) then
              if vRP.tryGetInventoryItem(user_id,idname,amount,true) then
                ...
                PerformHttpRequest(webhook.GiveItem, function(err, text, headers) end, 'POST', json.encode({username = 'SenturaCloud - Logs', content = dmessage}), { ['Content-Type'] = 'application/json' })
              else
                vRPclient.notify(player,{lang.common.invalid_value()})
              end
            else
  ...
  if user_id ~= nil then
    ...
    vRP.prompt(player,lang.inventory.trash.prompt({vRP.getInventoryItemAmount(user_id,idname)}),"",function(player,amount)
      ...
      if vRP.tryGetInventoryItem(user_id,idname,amount,false) then
        ...
        PerformHttpRequest(webhook.DestroyItem, function(err, text, headers) end, 'POST', json.encode({username = 'SenturaCloud - Logs', content = dmessage}), { ['Content-Type'] = 'application/json' })
...
function vRP.openInventory(source)
  ...
  if user_id ~= nil then
    ...
    if data then
      ...
      local menudata = {name=lang.inventory.title(),css={top="75px",header_color="rgba(204,104,0, 0.8)"}}
      -- add inventory info
...
local function build_itemlist_menu(name, items, cb)
  local menu = {name=name, css={top="75px",header_color="rgba(153, 136, 59,0.75)"}}
    ...
    if data.inventory ~= nil then
      if not chests[name] then
        ...
        vRP.getSData("chest:"..name, function(cdata)
          ...
          -- open menu
          local menu = {name=lang.inventory.chest.title(), css={top="75px",header_color="rgba(153, 136, 59,0.75)"}}
          -- take
