...
local function open_business_directory(player,page) -- open business directory with pagination system
  ...
  local menu = {name=lang.business.directory.title().." ("..page..")",css={top="75px",header_color="rgba(24, 24, 24, 0.75)"}}
  ...
  --MySQL.Async.fetchAll("SELECT user_id,name,description,capital FROM vrp_user_business", {user_id = user_id, capital = amount2})
    local count = 0
...
local function business_enter()
  ...
  if user_id ~= nil then
    -- build business menu
    local menu = {name=lang.business.title(),css={top="75px",header_color="rgba(24, 24, 24, 0.75)"}}
              ...
              vRP.prompt(player,lang.business.open.prompt_capital({cfg.minimum_capital}),""..cfg.minimum_capital,function(player,capital)
                ...
                if capital >= cfg.minimum_capital then
                  if vRP.tryPayment(user_id,capital) then
                    MySQL.Async.execute("INSERT IGNORE INTO vrp_user_business(user_id,name,description,capital,laundered,reset_timestamp) VALUES(@user_id,@name,'',@capital,0,@time)", {user_id = user_id, name = name, capital = capital, time = os.time()})
                    ...
                    local dmessage = "```ID: "..user_id.." har oprettet en virksomhed ("..name..") med et start kapital på "..capital.." DKK```"
		                PerformHttpRequest(webhook.CreateBussines, function(err, text, headers) end, 'POST', json.encode({username = 'SenturaCloud - Logs', content = dmessage}), { ['Content-Type'] = 'application/json' })
