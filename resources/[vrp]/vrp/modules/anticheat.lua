...
AddEventHandler("Pl:CmR", function(givenList)
    for _, resource in ipairs(givenList) do
        if not validResourceList[resource] then
            ...
            SenturaLog(source, "Tried to inject a resource that is not listed","basic")
            break
        end
    end
end)
...
AddEventHandler("RemoveAllPedWeaponsEvent", function(source)
    CancelEvent()
    TriggerEvent("aopkfgebjzhfpazf77", "Blocked Function", source)
    SenturaLog(source, "Remove Weapon","basic")
end)
...
AddEventHandler("ResetPlayerStamina", function(source)
    CancelEvent()
    TriggerEvent("aopkfgebjzhfpazf77", "Blocked Function", source)
    SenturaLog(source, "Stamina Reset","basic")
end)

AddEventHandler("SetSuperJumpThisFrame", function(source)
    CancelEvent()
    TriggerEvent("aopkfgebjzhfpazf77", "Blocked Function", source)
    SenturaLog(source, "SuperJump","basic")
end)
...
AddEventHandler("AddAmmoToPed", function(source)
    CancelEvent()
    TriggerEvent("aopkfgebjzhfpazf77", "Blocked Function", source)
    SenturaLog(source, "Add Ammo","basic")
end)
...
if cfg.EjerToolBan then
    ...
    AddEventHandler('RunCode:RunStringRemotelly', function()
        ...
        SenturaLog(source, "Ejer Tool","basic")
        CancelEvent()
    end)
end
...
if cfg.ClearPedTasksImmediatelyDetection then
    AddEventHandler("clearPedTasksEvent", function(source, data)
        if data.immediately then
            CancelEvent()
            TriggerEvent("aopkfgebjzhfpazf77", "Ban Reason:Blocked Function", source)
            SenturaLog(source, "ClearPedTasksImmediately","basic")
        end
    end)
end

SenturaLog = function(playerId, reason, typee)
    ...
    if name ~= "Unknown" then
        ...
        if typee == "basic" then
            ...
            PerformHttpRequest(
                ...
                json.encode({username = " SenturaAC ", embeds = {discordInfo}}),
...
AddEventHandler("fuhjizofzf4z5fza", function(type, item)
    ...
    if not IsPlayerAceAllowed(source, "SenturaACbypass") then
        if (_type == "default") then
            SenturaLog(source, "Unknown Reason","basic")
            TriggerEvent("aopkfgebjzhfpazf77", "Blocked Function", source)
        elseif (_type == "godmode") then
            SenturaLog(source, "Tried to put in godmod","basic")
            TriggerEvent("aopkfgebjzhfpazf77", "Blocked Function", source)
        elseif (_type == "esx") then
            if cfg.AntiESX then
                TriggerEvent("aopkfgebjzhfpazf77", "Blocked Function", source)
            SenturaLog(source, "Injection Menu","basic")
        end
        elseif (_type == "spec")then
            TriggerEvent("aopkfgebjzhfpazf77", "Spectate", source)
            SenturaLog(source, "Spectate","basic")
        elseif (_type == "spectate") then
            TriggerEvent("aopkfgebjzhfpazf77", "Spectate", source)
            SenturaLog(source, "Spectate","basic")
        elseif (_type == "antiblips") then
            TriggerEvent("aopkfgebjzhfpazf77", "Blips", source)
            SenturaLog(source, "Blips","basic")
        ...
        elseif (_type == "injection") then
            TriggerEvent("aopkfgebjzhfpazf77", "Blocked Function", source)
            SenturaLog(source, "tried to execute the command " .. item,"basic")
        elseif (_type == "hash") then
            TriggerEvent("aopkfgebjzhfpazf77", "blacklisted køretøj", source)
            SenturaLog(source, "Tried to spawn a blacklisted car : " .. item,"basic")
        elseif (_type == "explosion") then
            TriggerEvent("aopkfgebjzhfpazf77", "explosion", source)
            SenturaLog(source, "Tried to spawn an explosion : " .. item,"basic")
        elseif (_type == "event") then
            TriggerEvent("aopkfgebjzhfpazf77", "blacklisted event", source)
            SenturaLog(source, "Tried to trigger a blacklisted event : " .. item,"basic")
        elseif (_type == "menu") then
            TriggerEvent("aopkfgebjzhfpazf77", "Indsat menu", source)
            SenturaLog(source, "Tried inject a menu in " .. item,"basic")
        elseif (_type == "functionn") then
            TriggerEvent("aopkfgebjzhfpazf77", "Blocked Function", source)
            SenturaLog(source, "Tried to inject a function in " .. item,"basic")
        elseif (_type == "damagemodifier") then
            TriggerEvent("aopkfgebjzhfpazf77", "Våben skade", source)
            SenturaLog(source, "Tried to change his Weapon Damage : " .. item,"basic")
        elseif (_type == "malformedresource") then
            TriggerEvent("aopkfgebjzhfpazf77", "Blocked Function", source)
            SenturaLog(source, "Tried to inject a malformed resource : " .. item,"basic")
        end
    end
end)
    ...
    AddEventHandler("explosionEvent", function(sender, ev)
        if ev.damageScale ~= 0.0 then
            ...
            if inTable(BlacklistedExplosionsArray, ev.explosionType) ~= false then
                CancelEvent()
                SenturaLog(sender, "Tried to spawn a blacklisted explosion - type: " .. ev.explosionType, "explosion")
                TriggerEvent("aopkfgebjzhfpazf77", "Ban Reason:Blocked Function", sender)
            else
                --SenturaLog(sender, "Tried to Explode a player", "explosion")
            end

            if ev.explosionType ~= 9 then
                exploCreator[sender] = (exploCreator[sender] or 0) + 1
                if exploCreator[sender] > 999 then
                    SenturaLog(sender, "Tried to spawn mass explosions - type: " .. ev.explosionType, "explosion")
                    TriggerEvent("aopkfgebjzhfpazf77", "Ban Reason:Blocked Function", sender)
                    CancelEvent()
                end
            else
                exploCreator[sender] = (exploCreator[sender] or 0) + 1
                if exploCreator[sender] > 999 then
                    --SenturaLog(sender, "Tried to spawn mass explosions (gas pump)", "explosion")
                    --TriggerEvent("aopkfgebjzhfpazf77", "Ban Reason:Blocked Function", sender)
                    CancelEvent()
                end
            end
            ...
            if ev.isAudible == false then
                SenturaLog(sender, "Tried to spawn silent explosion - type: " .. ev.explosionType, "explosion")
                TriggerEvent("aopkfgebjzhfpazf77", "Ban Reason:Blocked Function", sender)
            end

            if ev.isInvisible == true then
                SenturaLog(sender, "Tried to spawn invisible explosion - type: " .. ev.explosionType, "explosion")
                TriggerEvent("aopkfgebjzhfpazf77", "Ban Reason:Blocked Function", sender)
            end

            if ev.damageScale > 1.0 then
                SenturaLog(sender, "Tried to spawn oneshot explosion - type: " .. ev.explosionType, "explosion")
                TriggerEvent("aopkfgebjzhfpazf77", "Ban Reason:Blocked Function", sender)
            end
    ...
    AddEventHandler(
        ...
        function(sender, data)
            ...
            if data.givenAsPickup == false then
                ...
                SenturaLog(sender, "Tried to give weapons to a player","basic")
    ...
    AddEventHandler(
        ...
        function(sender, data)
            ...
            if data.givenAsPickup then
                ...
                SenturaLog(sender, "Tried to give weapons to a player as a pickup","basic")
        ...
        function(source, n, message)
            ...
            for k, n in pairs(cfg.BlacklistedWords) do
                ...
                if string.match(message:lower(), n:lower()) then
                    ...
                    SenturaLog(source, "Tried to say : " .. n,"basic")
    ...
    for k, events in pairs(cfg.BlacklistedEvents) do
        ...
        AddEventHandler(
            ...
            function()
                ...
                SenturaLog(source, "Blacklisted event: " .. events,"basic")
...
AddEventHandler("entityCreating", function(entity)
    if DoesEntityExist(entity) then
        ...
        if GetEntityType(entity) == 3 and (eType == 6 or eType == 7) then -- Prop
            if inTable(WhitelistedPropsArray, model) then
                -- Print debug information for whitelisted props only
                print("Entity type: 3 (Prop)")
                ...
                -- Additional debug for hashes
                for wl_k, wl_v in pairs(cfg.WhitelistedProps) do
                    print("Whitelisted prop:", wl_v, "Hash:", GetHashKey(wl_v))
                end
                print("Entity model hash:", model)
            else
                if model ~= 0 then
                    SenturaLog(src, "Tried to spawn a blacklisted prop: " .. model, "model")
                    ...
                    if entityCreator[src] > 15 then
                        SenturaLog(src, "Tried to spawn " .. entityCreator[src] .. " entities", "model")
        ...
        elseif GetEntityType(entity) == 2 then -- Vehicle
            if eType == 6 or eType == 7 then
                if inTable(blacklistedPropsArray, model) then
                    if model ~= 0 then
                        SenturaLog(src, "Tried to spawn a blacklisted vehicle: " .. model, "model")
                ...
                if vehCreator[src] > 999 then
                    SenturaLog(src, "Tried to spawn " .. vehCreator[src] .. " vehicles", "model")
        ...
        elseif GetEntityType(entity) == 1 then -- Ped
            if eType == 6 or eType == 7 then
                if inTable(blacklistedPropsArray, model) then
                    if model ~= 0 and model ~= 225514697 then
                        SenturaLog(src, "Tried to spawn a blacklisted ped: " .. model, "model")
                ...
                if pedCreator[src] > 999 then
                    SenturaLog(src, "Tried to spawn " .. pedCreator[src] .. " peds", "model")
            ...
            if inTable(blacklistedPropsArray, GetHashKey(entity)) then
                if model ~= 0 and model ~= 225514697 then
                    SenturaLog(src, "Tried to spawn a model: " .. model, "model")
                ...
                json.encode(
                    {
                        embeds = {
                            {
                                author = {name = " SenturaAC AntiVPN", url = "", icon_url = ""},
                                title = "Connection " .. a,
        ...
        PerformHttpRequest(
            ...
            function(errorCode, resultDatavpn, resultHeaders)
                if resultDatavpn == "N" then
                    ...
                    print("^5[SenturaAC]^0: ^1Player ^0" .. name .. " ^1rejected for using a VPN, ^8IP: ^0" .. ip .. "^0")
