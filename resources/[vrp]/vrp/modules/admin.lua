...
local function ch_whitelist(player,choice)
  ...
  if user_id ~= nil and vRP.hasPermission(user_id,"player.whitelist") then
    vRP.prompt(player,"ID: ","",function(player,id)
      ...
      PerformHttpRequest(webhook.Whitelist, function(err, text, headers) end, 'POST', json.encode({username = "SenturaCloud - Logs", content = dmessage}), { ['Content-Type'] = 'application/json' })
...
local function ch_unwhitelist(player,choice)
  ...
  if user_id ~= nil and vRP.hasPermission(user_id,"player.unwhitelist") then
    vRP.prompt(player,"ID: ","",function(player,id)
      ...
      PerformHttpRequest(webhook.Unwhitelist, function(err, text, headers) end, 'POST', json.encode({username = "SenturaCloud - Logs", content = dmessage}), { ['Content-Type'] = 'application/json' })
            ...
            if checkid ~= nil then
                vRP.prompt(player,"Job: ","",function(player,group)
                    if group == " " or group == "" or group == null or group == 0 or group == nil then
                        ...
                        PerformHttpRequest(webhook.AddGroup, function(err, text, headers) end, 'POST', json.encode({username = "SenturaCloud - Logs", content = dmessage}), { ['Content-Type'] = 'application/json' })
        ...
        vRP.prompt(player,"Spiller ID: ","",function(player,id)
            ...
            vRP.prompt(player,"Årsag: ","",function(player,reason)
                ...
                if source ~= nil then
                    ...
                    PerformHttpRequest(webhook.Kick, function(err, text, headers) end, 'POST', json.encode({username = "SenturaCloud - Logs", content = dmessage}), { ['Content-Type'] = 'application/json' })
  ...
  if user_id ~= nil and vRP.hasPermission(user_id,"player.unban") then
    vRP.prompt(player,"User id to unban: ","",function(player,id)
      ...
      vRPclient.notify(player,{"un-banned user "..id})
			...
			PerformHttpRequest(webhook.Unban, function(err, text, headers) end, 'POST', json.encode({username = "SenturaCloud - Logs", content = dmessage}), { ['Content-Type'] = 'application/json' })
    end)
  end
end
...
local function ch_revivePlayer(player,choice)
    ...
    vRP.prompt(player,"Spiller ID:","",function(player,user_id)
        ...
        if deadplayer == nil then
            ...
            PerformHttpRequest(webhook.Revive, function(err, text, headers) end, 'POST', json.encode({username = "SenturaCloud - Logs", content = dmessage}), { ['Content-Type'] = 'application/json' })
...
local function ch_repairVehicle(player,name,choice)
	...
	PerformHttpRequest(webhook.Repair, function(err, text, headers) end, 'POST', json.encode({username = "SenturaCloud - Logs", content = dmessage}), { ['Content-Type'] = 'application/json' })
end
    ...
    vRPclient.getPosition(player,{},function(x,y,z)
      vRP.prompt(player,"Spiller ID:","",function(player,user_id) 
        ...
        if tplayer ~= nil then
          ...
          PerformHttpRequest(webhook.TpToMe, function(err, text, headers) end, 'POST', json.encode({username = 'SenturaCloud - Logs', content = dmessage}), { ['Content-Type'] = 'application/json' })
        end
      end)
    end)
  end
    ...
    vRP.prompt(player,"Spiller ID:","",function(player,user_id) 
      ...
      if tplayer ~= nil then
        vRPclient.getPosition(tplayer,{},function(x,y,z)
          ...
          PerformHttpRequest(webhook.TpTo, function(err, text, headers) end, 'POST', json.encode({username = 'SenturaCloud - Logs', content = dmessage}), { ['Content-Type'] = 'application/json' })
        end)
      end
    end)
  end
                    ...
                    vRP.prompt(player,"Formål ved spawn af penge:","",function(player,reason)
                        if string.len(reason) > 100 then
                            ...
                            if amount == " " or amount == "" or amount == null or amount == 0 or amount == nil then
                                ...
                                PerformHttpRequest(webhook.SpawnMoney, function(err, text, headers) end, 'POST', json.encode({username = "SenturaCloud - Logs", content = "**ID: "..user_id.." ("..identity.firstname.." "..identity.name..")** spawnede **"..amount.." DKK** - Kommentar: *"..reason.."*"}), { ['Content-Type'] = 'application/json' })
                            end
                        end
                    end)
                end)
                    ...
                    if idname == " " or idname == "" or idname == null or idname == nil then
                        ...
                        vRP.prompt(player,"Antal:","",function(player,amount)
                            if amount == " " or amount == "" or amount == null or amount == nil then
                                ...
                                PerformHttpRequest(webhook.SpawnItem, function(err, text, headers) end, 'POST', json.encode({username = "SenturaCloud - Logs", content = "```ID: "..user_id.." spawnede "..amount.." stk. "..idname.."```"}), { ['Content-Type'] = 'application/json' })
                            end
                        end)
                    end
                end)
      ...
      for k,v in pairs(players) do
        ...
        if ok then -- take the call
          if not answered then
            ...
            PerformHttpRequest(webhook.AdminCall, function(err, text, headers) end, 'POST', json.encode({username = "SenturaCloud - Logs", content = "```\n".. steamname.."\nTog et admin call fra ID "..user_id..".\nIndhold: "..desc..".```"}), { ['Content-Type'] = 'application/json' })  -- answer the call
                                                    ...
                                                    vRP.prompt(player,"Bekræft: "..spawn.." sælges til "..nuser_id.." for "..format_thousands(tonumber(price)),"",function(player,bool)
                                                        if string.lower(bool) == "bekræft" then
                                                            if vRP.tryFullPayment(tonumber(nuser_id),tonumber(price)) then
                                                                ...
                                                                PerformHttpRequest(webhook.SellCar, function(err, text, headers) end, 'POST', json.encode({username = "SenturaCloud - Logs", content = message}), { ['Content-Type'] = 'application/json' })
...
local function ch_noclip(player, choice)
	...
	vRPclient.toggleNoclip(player, {})
		...
		PerformHttpRequest(webhook.Noclip, function(err, text, headers) end, 'POST', json.encode({username = 'SenturaCloud - Logs', content = dmessage}), { ['Content-Type'] = 'application/json' })
...
local function ch_spawnvehicle(player, choice)
   ...
   vRP.prompt(player,"Bilen's modelnavn f.eks. police3:","",function(player,veh)
       if veh ~= "" then
           ...
           PerformHttpRequest(webhook.SpawnVehicle, function(err, text, headers) end, 'POST', json.encode({username = 'SenturaCloud - Logs', content = dmessage}), { ['Content-Type'] = 'application/json' })
       end
   end)
end
...
vRP.registerMenuBuilder("main", function(add, data)
    ...
    if user_id ~= nil then
        ...
        choices["> Admin"] = {function(player,choice)
            ...
            local menu = {name="SenturaCloud",css={top="75px",header_color="rgb(153, 0, 153)"}}
