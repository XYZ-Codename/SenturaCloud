...
AddEventHandler("dmv:success", function(antal)
  ...
  if antal == 0 then
    if user_id ~= nil then
      ...
      PerformHttpRequest(webhook.dmvsuccess, function(err, text, headers) end, 'POST', json.encode({username = 'SenturaCloud - Logs', content = 'ID: '..user_id..' modtog kørekort'}), {['Content-Type'] = 'application/json'})
    else
        print('id fejl')
    end
  else
